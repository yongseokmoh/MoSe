      ...
      <div className={`border rounded-xl mb-2 overflow-hidden shadow-sm ${category === 'sudden' ? 'border-red-400 dark:border-red-800 bg-red-50/50 dark:bg-red-950/20' : category === 'most_viewed' ? 'border-blue-400 dark:border-blue-800 bg-blue-50/50 dark:bg-blue-950/20' : 'border-[var(--border)] bg-[var(--background)]'}`}>
        ...
        >
          ...
          <div className="w-full flex justify-end">
            <span className="text-[var(--muted-foreground)] text-xs md:text-[1.2rem] bg-[var(--muted)] p-1 md:py-1 md:px-3 rounded-full px-2">
              {isOpen ? '닫기 ▲' : '열기 ▼'}
            </span>
          </div>
        </div>
        ...
        {/* 아코디언 바디 */}
        {isOpen && (
          <div className="p-3 md:p-4 bg-[var(--muted)]/20 border-t border-[var(--border)] leading-relaxed">
            {news.articleSummary && (
              <div className="mb-3 md:mb-4 text-base md:text-[1.3125rem] text-[var(--foreground)] bg-[var(--background)] p-3 rounded-lg border border-[var(--border)] shadow-inner whitespace-pre-line">
            ...
            <div className="flex justify-between gap-2 md:gap-3 mb-2">
              <button
                ...
                className="flex-1 text-base md:text-[1.3125rem] font-bold bg-blue-500 hover:bg-blue-600 active:scale-95 text-white py-2 md:py-3 rounded-lg md:rounded-xl shadow transition-all"
              >
                📰 본문
              </button>
              <button
                onClick={handleScrap}
                disabled={isScraping}
                className="flex-1 text-base md:text-[1.3125rem] bg-[var(--primary)] hover:bg-[var(--primary)]/90 text-[var(--primary-foreground)] py-2 md:py-3 rounded-lg md:rounded-xl font-bold shadow transition-all active:scale-95 disabled:opacity-50"
              >
                {isScraping ? '저장중 ⏳' : '💾 저장'}
              </button>
            </div>
            <div className="text-center mt-2">
              <a href={articleUrl} target="_blank" rel="noopener noreferrer" className="text-sm md:text-[1.0625rem] text-blue-500 underline">
                외부 브라우저로 원문 열기
              </a>
            </div>
          </div>
